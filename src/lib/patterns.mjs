...
export const PATTERNS = [
  ...
  { label: "OAuth Refresh Token", src: "[Rr]efresh\\s+token\\s*[:=]\\s*[\"']?[A-Za-z0-9+/_=]{16,}", validate: null, context: true },
  ...
  { label: "Generic Secret/Password Assignment", src: "(?:api[_-]?key|apikey|access[_-]?key|client[_-]?secret|private[_-]?key|secret|(?<!Refresh\\s)token|password|passwd|pwd|auth[_-]?token)[\"']?\\s*[:=]\\s*[\"']?[A-Za-z0-9+/=_\\-]{12,}[\"']?", validate: null },
